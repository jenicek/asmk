...
cdef float c_hamming_dist_uint_arr(unsigned int[::1] n1, unsigned int[::1] n2, float normalization) nogil:
...
cdef unsigned int binarize_and_pack_uint32(float[::1] arr, Py_ssize_t length, int threshold) nogil:
...
def hamming_dist_packed(unsigned int[::1] n1, unsigned int[::1] n2, float normalization = 0):
    ...
    return c_hamming_dist_uint_arr(n1, n2, normalization)
...
def hamming_cdist_packed(unsigned int[:,::1] arr1, unsigned int[:,::1] arr2, float normalization = 0):
    ...
    >> hamming_dist_packed(np.array([[3], [1]], dtype=np.uint32), np.array([[1], [2]], dtype=np.uint32), 2)
    array([[0.5, 0.5],
           [0. , 1. ]], dtype=float32)
    """
    ...
    for i in range(dim0):
        for j in range(dim1):
            result_view[i, j] = c_hamming_dist_uint_arr(arr1[i], arr2[j], normalization)
...
def binarize_and_pack(float[::1] arr, int threshold = 0):
    ...
    for i in range(dim-1):
        result_view[i] = binarize_and_pack_uint32(arr[offset:], 32, threshold)
        offset += 32

    # Last iteration
    tmp = binarize_and_pack_uint32(arr[offset:], dim_orig - offset, threshold)
...
def binarize_and_pack_2D(float[:,::1] arr, int threshold = 0):
    ...
    >> binarize_and_pack_((np.random.rand(2, 10) - 0.5).astype(np.float32))
    """
    ...
    for i in range(dim0):
        ...
        for j in range(dim1-1):
            result_view[i,j] = binarize_and_pack_uint32(arr[i][offset:], 32, threshold)
            offset += 32

        # Last iteration
        tmp = binarize_and_pack_uint32(arr[i][offset:], dim1_orig - offset, threshold)
